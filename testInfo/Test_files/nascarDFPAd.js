 ...
 */
 ...
 if(window.location.href.indexOf("m.nascar.com")>-1){
      domain= "m.nascar.com";
 }
if(window.isPrebidConbfigured){
    // Make a call to Pribid.js
        jQuery.ajax({
            type: "GET",
            url: "https://"+domain+"/wp-content/themes/ndms/js/ads/prebid4.3.0.js",
            success: function(){},
            dataType: "script",
            cache: true
        });
    ...
    var nascarBreakPoint=getBreakPoint();
    ...
    var NascarAd = (function() {
                ...
                xhr.onreadystatechange = function() {
                    ...
                    if (xhr.readyState === 4) {
                        ...
                        if(typeof xhr.getResponseHeader === 'function'){
                            ...
                            if (responseContentType && responseContentType.indexOf('json') > 0){
        ...
        function _updateTargets(root, id) {
            ...
            _updateBreakpoint();
            // Add Adobe ID as Cust Param in ad calls
            var amId= NascarAd.readCookie('aam_uuid')
            if(amId) {
                _targets.pageTargets['aamId'] = amId;
            }
        }
        ...
        function _updateBreakpoint(){
            _targets.pageTargets['breakpoint'] = getBreakPoint();
        }
        ...
        function buildPreBidSlotArr(adSlotArray){
            var preBidSlotArr=[];
            var tempArr = [];
            for(i in adSlotArray ) {
                if(adSlotArray[i]['adunit_id']) {
                    tempArr = createBidSlot(adSlotArray[i]);
                    if(tempArr != null ) {
                        preBidSlotArr.push(tempArr);
                    }
                }
            }
            /*if( _videoAdTagData &&
                _videoAdTagData.videoAndCompanionSlots &&
                _videoAdTagData.videoAndCompanionSlots.video &&
                _videoAdTagData.videoAndCompanionSlots.video.length > 0) {
                tempArr = createBidSlot(_videoAdTagData.videoAndCompanionSlots.video[0]);
                if(tempArr != null){
                    tempArr['code'] = '/' + _options.networkId + '/' + _videoAdTagData.adId+'/'+tempArr['code'];
                	preBidSlotArr.push(tempArr);
            	}
            }*/
            return preBidSlotArr;
        }

        function invokePrebidWorkFlow(data, callBackFunc){
            var adSlots = buildPreBidSlotArr(data);
            _logDebug('#### Prebit Adunits ####: ');
            _logDebug(JSON.stringify(adSlots));
            if(window.pbjs) {
               /* var customPriceGranularity = {
                  'buckets': [
                    { "max": 5.0,"increment": 0.05,"min": 0.0},
                    { "max": 10.0,"increment": 0.1,"min": 5.0},
                       { "max": 20.0,"increment": 0.5,"min": 10.0},
                    { "max": 50.0,"increment": 1.0,"min": 20.0}
                  ]
                };*/
                ...
                window.pbjs.que.push(function () {
                    _logDebug('### Prebid JS : add adunits to Predid.js & invoke pbjs.requestBids ####');
                    window.pbjs.addAdUnits(adSlots);
                    window.pbjs.requestBids({
                        bidsBackHandler: callBackFunc,
                ...
                setTimeout(function () {
                    _logDebug('### Prebid JS : Time out Executed ####');
                    _dispatchQueueOnceReady();
                }, FAILSAFE_TIMEOUT);
            }else{
                _logDebug('#### Prebid is not defined on the page  ####: ');
            }
        }

        function createBidSlot(slotData){
            var tempArr = {};
            var placementId=null;
            var placementOpenxId=null;
            var placementSovrnId=null;
            var placementTripleLiftId=null;
            var sizes =[];
            var preBidMappingArray = slotData['placementIds'];
            var preBidOpenxMappingArray = slotData['placementOpenxIds'];
            var preBidSovrnMappingArray = slotData['placementSovrnIds'];
            var preBidTripleLiftMappingArray = slotData['placementTripleLiftIds'];

            if(preBidMappingArray) {
                preBidMappingArray = preBidMappingArray.split('|');
                for (i in preBidMappingArray) {
                    if (typeof preBidMappingArray[i].indexOf == 'function'
                        && preBidMappingArray[i].indexOf(nascarBreakPoint) > -1) {
                        placementId = preBidMappingArray[i].split(':')[1];
            ...
            if(preBidOpenxMappingArray) {
                preBidOpenxMappingArray = preBidOpenxMappingArray.split('|');
                for (i in preBidOpenxMappingArray) {
                    if (typeof preBidOpenxMappingArray[i].indexOf == 'function'
                        && preBidOpenxMappingArray[i].indexOf(nascarBreakPoint) > -1) {
                        placementOpenxId = preBidOpenxMappingArray[i].split(':')[1];
                        placementOpenxId = placementOpenxId.trim();
                        break;
                    }
                }
            }

            if(preBidSovrnMappingArray) {
                preBidSovrnMappingArray = preBidSovrnMappingArray.split('|');
                for (i in preBidSovrnMappingArray) {
                    if (typeof preBidSovrnMappingArray[i].indexOf == 'function'
                        && preBidSovrnMappingArray[i].indexOf(nascarBreakPoint) > -1) {
                        placementSovrnId = preBidSovrnMappingArray[i].split(':')[1];
                        placementSovrnId = placementSovrnId.trim();
                        break;
                    }
                }
            }

            if(preBidTripleLiftMappingArray) {
               preBidTripleLiftMappingArray = preBidTripleLiftMappingArray.split('|');
               for (i in preBidTripleLiftMappingArray) {
                   if (typeof preBidTripleLiftMappingArray[i].indexOf == 'function'
                       && preBidTripleLiftMappingArray[i].indexOf(nascarBreakPoint) > -1) {
                       placementTripleLiftId = preBidTripleLiftMappingArray[i].split(':')[1];
                       placementTripleLiftId = placementTripleLiftId.trim();
                       break;
                   }
               }
           }

            if((typeof placementId != 'undefined' && placementId != null) ||
                (typeof placementOpenxId != 'undefined' && placementOpenxId != null) ||
                (typeof placementSovrnId != 'undefined' && placementSovrnId != null) ||
                (typeof placementTripleLiftId != 'undefined' && placementTripleLiftId != null)
              ) {
                if(slotData['responsive']){
                    if (nascarBreakPoint == 'desktop' && slotData['responsive'][0][1][0] != "suppress") {
                        sizes = slotData['responsive'][0][1];
                    } else if (nascarBreakPoint == 'tablet') {
                        if (slotData['responsive'][1][1][0] != "suppress") {
                            for (i in slotData['responsive'][1][1]) {
                                sizes.push(slotData['responsive'][1][1][i]);
                            }
                        }
                        if (slotData['responsive'][2][1][0] != "suppress") {
                            for (i in slotData['responsive'][2][1]) {
                                sizes.push(slotData['responsive'][2][1][i]);
                            }
                        }

                    } else if (nascarBreakPoint == 'mobile') {
                        if (slotData['responsive'][3][1][0] != "suppress") {
                            for (i in slotData['responsive'][3][1]) {
                                sizes.push(slotData['responsive'][3][1][i]);
                            }
                        }
                        if (slotData['responsive'][4][1][0] != "suppress") {
                            for (i in slotData['responsive'][4][1]) {
                                sizes.push(slotData['responsive'][4][1][i]);
                            }
                        }
                    }
                ...
                if(!slotData['slotid']) {
                    if(sizes.length>0) {
                        console.log(tempArr['bids']);
                        tempArr['code'] = slotData['slot_id'];
                        tempArr['mediaTypes'] = {'banner': {'sizes': sizes}};
                        tempArr['bids'] = [];

                        if(typeof placementId != 'undefined' && placementId != null){
                            tempArr['bids'].push({'bidder': 'appnexus', 'params': {placementId: placementId}});
                        }
                        if(typeof placementOpenxId != 'undefined' && placementOpenxId != null){
                            tempArr['bids'].push({ 'bidder': 'openx', 'params':{delDomain: 'nascardm-d.openx.net', unit: placementOpenxId }});
                        }
                        if(typeof placementSovrnId != 'undefined' && placementSovrnId != null){
                            tempArr['bids'].push({ 'bidder': 'sovrn', 'params':{ tagid: placementSovrnId }});
                        }
                        if(typeof placementTripleLiftId != 'undefined' && placementTripleLiftId != null){
                            tempArr['bids'].push({ 'bidder': 'triplelift', 'params':{ inventoryCode: placementTripleLiftId }});
                        }
                        return tempArr;
                    }
                    return null;
                }else{
                    tempArr['code'] = slotData['slotid'];
                    tempArr['mediaTypes'] = {'video': {'playerSize': [640, 480],'context': 'instream'}};
                    tempArr['bids'] = [{'bidder': 'appnexus', 'params': {placementId: placementId,video: {
                        'skipppable': true,
        ...
        function _dispatchQueueOnceReady() {
            _logDebug('### NASCAR Ad Call generated : _dispatchQueueOnceReady ');
            if (_isReady()) {
                if(window.isPrebidConbfigured) {
                    if (window.pbjs.initAdserverSet) {
                        _logDebug('### NASCAR Ad Call is already generated hence ignoring this invocation : _dispatchQueueOnceReady ');
                        return;
                    }
        ...
        var _VIDEO_AD_TAG_TEMPLATE = 'https://pubads.g.doubleclick.net/gampad/ads?ppid='+_getPIDPageTargeting()+'&sz=[sizes][companionsSizes]&iu=[ad_unit]&impl=s&gdfp_req=1&env=vp&output=vast&unviewed_position_start=1&url=[referrer_url]&description_url=https%3A%2F%2Fnascar.com'+window.location.pathname+'&correlator=[timestamp]';
        var _COMPANION_SIZES_TAG_PART_TEMPLATE = '&ciu_szs=[companionsSizes]';
        var _TARGETING_TAG_PART_TEMPLATE = '&cust_params=';
        function createVideoAdTag(){
          var temp_video_ad_tag = _VIDEO_AD_TAG_TEMPLATE;
            ...
            try {
                if (nascarBrightCoveCmsid && !(temp_video_ad_tag.indexOf('cmsid')>-1)) {
                    temp_video_ad_tag = temp_video_ad_tag + '&vid=' + nascarVideoid;
                    temp_video_ad_tag = temp_video_ad_tag + '&cmsid=' + nascarBrightCoveCmsid;
                }
            }catch(e){
                console.log('nascarBrightCoveCmsid is not defined');
            }
            // Added header bidding for videos : Start
           /* if( window.isPrebidConbfigured && window.pbjs) {
                var tempVideoAdUnit=null;
                jQuery(window.pbjs.adUnits).each(function(i,adunit){
                    if(adunit != null && adunit['code'].indexOf('/'+_options.networkId)>-1){
                        tempVideoAdUnit=adunit;
                        return false;
                    }
                });
                if(tempVideoAdUnit != null){
                    var videoUrl = pbjs.adServers.dfp.buildVideoUrl({
                        adUnit:tempVideoAdUnit,
                        url:_VIDEO_AD_TAG_TEMPLATE
                    });
                    _VIDEO_AD_TAG_TEMPLATE = videoUrl;
                }
            }*/
            if(temp_video_ad_tag.indexOf('cust_params')>-1){
              temp_video_ad_tag = temp_video_ad_tag+'%26'+targeting;
            }else{
              temp_video_ad_tag = temp_video_ad_tag+_TARGETING_TAG_PART_TEMPLATE+targeting;
            }
            // Added header bidding for videos : End

            return temp_video_ad_tag
        ...
        function _loadSlotsData(){
           ...
           window.moatDataReady = false; // Set before dropping Moat tag to prevent race condition
            ...
            if(adUnitId){
                _ajax({
                    ...
                    if(error){
                        ...
                        function checkForMoatAndMakeAdRequest() {
                            if (moatDataReady) {
                                window.console.log("Moat Yield Intelligence: data available, attempting to set targeting");
                                _logDebug("AJAX data", data);
                                data = _adjustData(data);
                                _logDebug("Adjusted AJAX data", data);
                            } else {
                                setTimeout(checkForMoatAndMakeAdRequest, 1000);
                            }
                        }
                        checkForMoatAndMakeAdRequest();
                    }
                });
            ...
            for(var i = 0, l = data[0].adslot.length; i < l ; i++){
                ...
                if(slotData.slotid.indexOf('video') == -1){
                    entry = {
                        ...
                        placementIds : slotData['placementIds'],
                        placementOpenxIds : slotData['placementOpenxIds'],
                        placementSovrnIds : slotData['placementSovrnIds'],
                        placementTripleLiftIds : slotData['placementTripleLiftIds']
                    };
                    result.push(entry);
                }
            }
            NascarAd.registry.push(result);
            if(window.isPrebidConbfigured) {
                // Invoking Prebid Workflow
                _logDebug('#### initiate Prebid workflow on Page Load  ####: ');
                invokePrebidWorkFlow(result,_dispatchQueueOnceReady);
            }
            try{
                _logDebug("NASCAR Ads Video Adslot created: ", _videoAdTagData);
                var event = new CustomEvent("nascarAdVideoAdSlot", {
                    detail: _videoAdTagData
                });
                document.dispatchEvent(event);
            } catch (error) {
                _logWarn("error dispatching custom Event: nascarAdVideoAdSlot", error)
            }
            return result;
        }
        ...
        function _slotsizesToSizes(slotsizes){
            ...
            for(var i = 0; i< arr.length ; i ++ ){
                if (uniqueCheck.indexOf(arr[i]) == -1){
                    ...
                    if(size.length > 1) {// Added If check to pass Fluid as valid size in slot mapping
        ...
        function requestAndRenderAds(options) {
            ...
            if (_slotQueue.length > 0) {
                ...
                async.parallelWithTimeout("preProcessSlot", _options.dispatchCallbackTimeoutInMilliseconds, preSlotProcessCallbacks, function(msg) {
                    ...
                    if(window.isPrebidConbfigured && window.pbjs){
                        _logDebug('###$$ Prebid JS : Adding Custom Param ####$$');
            ...
            _ajax({
                ...
                if(error){
                    ...
                    if(!window.isPrebidConbfigured) {
                        ...
                        if(window.isPrebidConbfigured) {
                            window.isPrebidConbfigured=false;
                        }
                    }
                }
            });
        ...
        function refresh(ids, options) {
            ...
            var __callback = function() {
                if (_isFocused || !_options.refreshOnFocusOnly) {
                    ...
                    async.parallelWithTimeout("preRefresh", _options.refreshCallbackTimeoutInMilliseconds, preRefreshCallbacks, function(msg) {
                        ...
                        if(window.isPrebidConbfigured && window.pbjs){
                            _logDebug('###$$ Prebid JS Refresh Slots : Adding Custom Param ####');
                            window.pbjs.setTargetingForGPTAsync && window.pbjs.setTargetingForGPTAsync();
                        }
            ...
            var tempSlots=[];
            if(ids.length == 0){
                tempSlots=NascarAd.registry[0];
            }else{
                for(i in ids){
                    for(j in NascarAd.registry[0]) {
                        if (NascarAd.registry[0][j]['slot_id']==ids[i]) {
                            tempSlots.push(NascarAd.registry[0][j]);
                            break;
                        }
                    }
                }
            ...
            if (options.interval && parseInt(options.interval) > 0) {
                ...
                if(window.isPrebidConbfigured) {
                    // Invoking Prebid Workflow
                    _logDebug('#### initiate Prebid workflow on Refresh Ads  ####: ');
                    invokePrebidWorkFlow(tempSlots, __runner(interval));
                }else{
                    __runner(interval);
                }
            } else {
                if(window.isPrebidConbfigured) {
                    // Invoking Prebid Workflow
                    _logDebug('#### initiate Prebid workflow on Refresh Ads  ####: ');
                    invokePrebidWorkFlow(tempSlots,__callback);
                }else{
                    __callback();
                }
            }
        ...
        function _isElementInViewport (el) {
            ...
            scrolled=scrolled+viewHight+viewHight/4;

            result = (
                rect.bottom >= 0 &&
                rect.right >= 0 &&
                scrolled >= el.offsetTop &&
        ...
        return ({
            ...
            getPIDPageTargeting:_getPIDPageTargeting
        })
    })();
